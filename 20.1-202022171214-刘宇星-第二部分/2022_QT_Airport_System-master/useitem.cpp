...
void useritem::AddItem(Customer1 *item)
{
    mItems.push_back(item);
}

void useritem::ClearData()
{
    for(int i=0;i<mItems.size();i++)
    {
        Customer1* p = mItems[i];
...
void useritem::OpenDb(QString fileName)
{
...
void useritem::SaveToDb(QString fileName)
{
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName(fileName);
    if(db.open())
...
void useritem::InsertToDb(Customer1 *p)
{
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
...
void useritem::DeleteFromDb(QString name)
{
    QSqlDatabase db = QSqlDatabase::addDatabase("QSQLITE");
    db.setDatabaseName(mDefaultDB);
    if(db.open())
...
void useritem::onMenuOpen(bool checked)
{
...
void useritem::onMenuSave(bool checked)
{
...
{
    userticket dlg(NULL);
    connect(&dlg,&userticket::sigAddItem,this,useritem::slotAddItem);
    dlg.exec();
}
...
void useritem::slotAddItem(Customer1 *p)
{
    mItems.push_back(p);
